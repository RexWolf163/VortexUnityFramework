...
namespace Vortex.UI
{
    ...
    public static partial class UIController
    {
        ...
        private static IUserInterface GetUI<T>() where T : IUserInterface
        {
            ...
            if (wnd == null)
            {
                LogController.Log(new LogData(LogLevel.Error, $"UI {typeof(T).Name} not found", "UIController"));
                return null;
            }
        ...
        private static IUserInterface GetUI(Type type)
        {
            ...
            if (wnd == null)
            {
                LogController.Log(new LogData(LogLevel.Error, $"UI {type.Name} not found", "UIController"));
                return null;
            }
