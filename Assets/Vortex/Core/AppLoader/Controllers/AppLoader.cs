...
namespace Vortex.Core.Loading.Controllers
{
    /// <summary>
    ...
    public static class AppLoader
    {
        ...
        /// Модель данных приложения
        /// </summary>
        private static AppModel Data => _data ??= AppController.Data;
        ...
        public static void Register<T>() where T : ISystemController, new()
        {
            var type = typeof(T);
            Data.controllers.AddNew(type, new T());
        ...
        {
            AppController.Data.OnExit += Destroy;
            await Task.Run(() => Loading(Token));
            Data.SetState(AppStates.Running);
        }
